...
def create_agent_briefing_admin_test(
    ...
    def fallback_response(reason: str, provider_error: str | None = None):
        saved_report_lines = "\n".join([
            f"- {r.get('title')} ({r.get('module_key')}) — {r.get('summary')}"
            for r in saved_reports[:5]
        ]) or "No saved reports available."
...
"""
    ...
    if not openai_api_key:
        return fallback_response("openai_api_key_missing")
...
"""
    ...
    try:
        ...
        return fallback_response("model_provider_timeout_or_connection_error", str(e))

    if res.status_code not in (200, 201):
        return fallback_response("model_provider_unavailable", res.text)
